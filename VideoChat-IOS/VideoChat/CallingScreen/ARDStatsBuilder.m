...
- (NSMutableDictionary *)statsStringDictionary {
    NSMutableDictionary *statsStringDictionaryResult = [[NSMutableDictionary alloc] init];
    NSMutableString *systemStatsResult = [NSMutableString string];
    NSString *systemStatsFormat = @"(cpu)%ld%%\n";
    [systemStatsResult appendString:[NSString stringWithFormat:systemStatsFormat,
                          (long)ARDGetCpuUsagePercentage()]];
    [statsStringDictionaryResult setObject:systemStatsResult forKey:@"systemStats"];
    
    // Connection stats.
    NSMutableString *connStatsResult = [NSMutableString string];
    NSString *connStatsFormat = @"CN %@ms | %@->%@/%@ | (s)%@ | (r)%@\n";
    [connStatsResult appendString:[NSString stringWithFormat:connStatsFormat,
                          _connRtt,
                          _localCandType, _remoteCandType, _transportType,
                          _connSendBitrate, _connRecvBitrate]];
    [statsStringDictionaryResult setObject:connStatsResult forKey:@"connStats"];
    
    // Video send stats.
    NSMutableString *videoSendResult = [NSMutableString string];
    NSString *videoSendFormat = @"VS (input) %@x%@@%@fps | (sent) %@x%@@%@fps\n"
    "VS (enc) %@/%@ | (sent) %@/%@ | %@ms | %@\n"
    "AvgQP (past %d encoded frames) = %d\n ";
    int avgqp = [self calculateAvgQP];
    
    [videoSendResult appendString:[NSString stringWithFormat:videoSendFormat,
                          _videoInputWidth, _videoInputHeight, _videoInputFps,
                          _videoSendWidth, _videoSendHeight, _videoSendFps,
                          _actualEncBitrate, _targetEncBitrate,
                          _videoSendBitrate, _availableSendBw,
                          ...
                          _videoSendCodec,
                          _framesEncoded - _oldFramesEncoded, avgqp]];
    [statsStringDictionaryResult setObject:videoSendResult forKey:@"videoSend"];
    
    // Video receive stats.
    NSMutableString *videoReceiveResult = [NSMutableString string];
    NSString *videoReceiveFormat =
    @"VR (recv) %@x%@@%@fps | (decoded)%@ | (output)%@fps | %@/%@ | %@ms\n";
    [videoReceiveResult appendString:[NSString stringWithFormat:videoReceiveFormat,
                          _videoRecvWidth, _videoRecvHeight, _videoRecvFps,
                          _videoDecodedFps,
                          _videoOutputFps,
                          _videoRecvBitrate, _availableRecvBw,
                          _videoDecodeMs]];
    [statsStringDictionaryResult setObject:videoReceiveResult forKey:@"videoReceive"];
    
    // Audio send stats.
    NSMutableString *audioSendResult = [NSMutableString string];
    NSString *audioSendFormat = @"AS %@ | %@\n";
    [audioSendResult appendString:[NSString stringWithFormat:audioSendFormat,
                          _audioSendBitrate, _audioSendCodec]];
    [statsStringDictionaryResult setObject:audioSendResult forKey:@"audioSend"];
    
    // Audio receive stats.
    NSMutableString *audioReceiveResult = [NSMutableString string];
    NSString *audioReceiveFormat = @"AR %@ | %@ | %@ms | (expandrate)%@";
    [audioReceiveResult appendString:[NSString stringWithFormat:audioReceiveFormat,
                          _audioRecvBitrate, _audioRecvCodec, _audioCurrentDelay,
                          _audioExpandRate]];
    [statsStringDictionaryResult setObject:audioReceiveResult forKey:@"audioReceive"];
    
    return statsStringDictionaryResult;
}
